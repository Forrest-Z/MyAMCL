...
PfPdfGaussianT *PfPdfGaussianAlloc(PfVectorT x, PfMatrixT cx)
{
  ...
  pdf = (PfPdfGaussianT*) calloc(1, sizeof(PfPdfGaussianT));
  ...
  PfMatrixUnitary(&pdf->cr, &cd, pdf->cx);
...
Scalar pf_pdf_gaussian_value(PfPdfGaussianT *pdf, PfVectorT x)
{
  int i, j;
  PfVectorT z;
  Scalar zz, p;
  
  z = PfVectorSub(x, pdf->x);
...
PfVectorT PfPdfGaussianSample(PfPdfGaussianT *pdf)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    r.v[i] = PfRanGaussian(pdf->cd.v[i]);
  }
